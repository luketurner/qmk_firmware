...
C_SIZE mapKeys(C_SIZE chord, bool lookup) {
  ...
  for (int i = 0; i < keyLen; i++) {
    if (keyDict[i].chord == chord && layer_state_is(keyDict[i].layer)) {
  ...
  for (int i = 0; i < stringLen; i++) {
    ...
    if (fromPgm.chord == chord && layer_state_is(fromPgm.layer)) {
      if (!lookup) {
  ...
  for (int i = 0; i < funcsLen; i++) {
    if (funDict[i].chord == chord && layer_state_is(funDict[i].layer)) {
  ...
  for (int i = 0; i < specialLen; i++) {
    if (spcDict[i].chord == chord && layer_state_is(spcDict[i].layer)) {
      if (!lookup) {
