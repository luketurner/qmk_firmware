...
#define P_KEYMAP(layer, chord, keycode)	 			{chord, layer, keycode},

#define K_KEYMAP(layer, chord, name, ...)			{chord, layer, (PGM_P)&name},
#define K_ACTION(layer, chord, name, ...)			const uint8_t	name[] PROGMEM = __VA_ARGS__;

#define S_KEYMAP(layer, chord, name, string) 	{chord, layer, (PGM_P)&name},
#define S_ACTION(layer, chord, name, string)		const char name[] PROGMEM = string;

#define X_KEYMAP(layer, chord, name, func)			{chord, layer, name},
#define X_ACTION(layer, chord, name, func)			void name(void) {func}

#define Z_KEYMAP(layer, chord, act, arg)				{chord, layer, act, arg},
